<template>
    <div class="w-9/12 mx-auto">
        <div class="flex  bg-gray-900 rounded-lg text-lg italic text-white p-5 m-2">
            ...
            <span class="w-5/6 px-5 text-2xl"><span class="text-green-500 text-4xl">Star</span> Your Social Network</span>
            <img :src="picture" alt="Profil_Image" class="size-[2vw] text-right mx-2 rounded-[20px]">
            <span class="text-right mx-2 text-green-500 cursor-pointer text-2xl" @click="showPopup">{{ username }}</span>
            <span class="text-right mx-12 cursor-pointer" @click.prevent="usersStore.logout()">Logout</span>
        </div>
